...
app.use('/hyperchat', express.static('public'));
...
wss.addListener('request', function(request) {
    ...
    connection.on('message', function(message) {
        if (message.type === 'utf8') {
            ...
            switch(data.type) {
                ...
                break;
                case "chat":
                    console.log(`${player.id}[${player.name}] :  `, data.data.message);
    ...
    connection.on('close', function(connection) {
        ...
        console.log(`${player.id}[${player.name}] has left.`);
        db.broadcastMessage({id: "Server", name: "Server"}, `${player.name || player.id}] has left.`);
